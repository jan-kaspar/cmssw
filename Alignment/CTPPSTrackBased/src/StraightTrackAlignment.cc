  ...
  const DetSetVector<CTPPSDiamondRecHit> &hitsDiamond, const DetSetVector<CTPPSPixelRecHit> &hitsPixel)
{
    ...
    for (const auto &pattern : { pv[idx_U], pv[idx_V] })
    {
      for (const auto &hitsDetSet : pattern.getHits())
      {
        ...
        for (auto &hit : hitsDetSet)
          hitSelection.emplace_back(Hit(hitsDetSet.detId(), 2, hit.getPosition(), hit.getSigma(), z));
      }
    }
  }
  ...
  for (const auto &pv : hitsPixel)
  {
    ...
    for (const auto &h : pv)
    {
      ...
      double x_unc = sqrt(h.getError().xx());
      double y_unc = sqrt(h.getError().yy());

      // TODO: remove this code once solved upstream (pixel reconstruction)
      if (x_unc <= 0.)
        x_unc = 10E-3;  // mm
      if (y_unc <= 0.)
        y_unc = 10E-3;  // mm

      hitSelection.emplace_back(Hit(pv.detId(), 1, h.getPoint().x(), x_unc, z));
      hitSelection.emplace_back(Hit(pv.detId(), 2, h.getPoint().y(), y_unc, z));
    }
  }
