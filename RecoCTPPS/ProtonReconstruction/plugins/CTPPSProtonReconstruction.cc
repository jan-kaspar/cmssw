  ...
  algorithm_(opticsFileBeam1_.fullPath(), opticsFileBeam2_.fullPath(), beamConditions_)
{
  ...
  edm::FileInPath alignmentFile("RecoCTPPS/ProtonReconstruction/data/collect_alignments.out");
  alignmentCollection_.Load(alignmentFile.fullPath());
...
void CTPPSProtonReconstruction::produce(Event& event, const EventSetup&)
{
  ...
  if (ret != 0)
  {
    ...
    printf("ERROR: could not find fill info for run %u.\n", event.id().run());
    return;
  }
  ...
  {
    event.put(move(output));
    printf("ERROR: could not find alignment for tag %s.\n", fillInfo.alignmentTag.c_str());
    return;
  }
  ...
  // run reconstruction per RP (old approach)
